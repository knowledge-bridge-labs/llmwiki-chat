...
`llmwiki-chat` provides a browser-safe quickstart panel that displays status,
copyable commands, and verification actions. It does not launch local processes
or install runtime frameworks.
...
- The first-run path is clearer without weakening browser safety.
